...
def ejecutar_rotacion(engine) -> "pd.DataFrame":
    """Ejecuta análisis de rotación y retorna DataFrame. No guarda en BD."""
    df_ventas, df_inv, df_cat = leer_datos(engine)
    df = calcular_rotacion(df_ventas, df_inv, df_cat)
    # Incluye tienda_id para que la PK compuesta (sku_id, tienda_id) sea única
    cols = ["sku_id", "tienda_id", "tasa_rotacion_anual", "dsi",
            "frecuencia_venta", "indice_velocidad", "clasificacion_rotacion"]
    cols = [c for c in cols if c in df.columns]
    result = df[cols].copy()
    for col in ["tasa_rotacion_anual", "dsi", "frecuencia_venta", "indice_velocidad"]:
