...
def ejecutar_rentabilidad(engine) -> "pd.DataFrame":
    """Ejecuta análisis de rentabilidad y retorna DataFrame. No guarda en BD."""
    df_ventas, df_cat = leer_datos(engine)
    df = calcular_rentabilidad(df_ventas, df_cat)
    # Incluye tienda_id para que la PK compuesta (sku_id, tienda_id) sea única
    cols = ["sku_id", "tienda_id", "margen_porcentual", "rentabilidad_total",
            "indice_rentabilidad", "clasificacion"]
    cols = [c for c in cols if c in df.columns]
    result = df[cols].copy()
    for col in ["margen_porcentual", "rentabilidad_total", "indice_rentabilidad"]:
