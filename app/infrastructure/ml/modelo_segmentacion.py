...
# ─────────────────────────────────────────
def ejecutar_segmentacion(engine):
    """
    Ejecuta segmentación y retorna (df_skus, df_tiendas).
    No guarda en BD — lo hace el repositorio.
    """
    df_ventas, df_inventario, df_tiendas = leer_datos(engine)

    # Para evitar que la función interna guarde en BD, re-ejecutamos la lógica:
    from sqlalchemy import text as _text
    import numpy as _np
    from sklearn.preprocessing import StandardScaler as _SS
    from sklearn.cluster import KMeans as _KM

    # ABC segmentation — leer_datos retorna "cantidad_total" (SUM(quantity))
    ventas_sku = df_ventas.groupby("sku_id")["cantidad_total"].sum().reset_index()
    ventas_sku = ventas_sku.sort_values("cantidad_total", ascending=False)
    ventas_sku["participacion"] = ventas_sku["cantidad_total"] / ventas_sku["cantidad_total"].sum() * 100
    ventas_sku["acumulado"] = ventas_sku["participacion"].cumsum()
    ventas_sku["segmento_abc"] = ventas_sku["acumulado"].apply(
        lambda x: "A" if x <= 70 else ("B" if x <= 90 else "C")
    )
    df_skus = ventas_sku[["sku_id", "participacion", "acumulado", "segmento_abc"]]

    # Store segmentation (simplified)
    metricas = df_ventas.groupby("tienda_id").agg(
        ventas_totales=("cantidad_total", "sum"),
        venta_promedio=("cantidad_total", "mean"),
        num_skus=("sku_id", "nunique"),
    ).reset_index()

    if len(metricas) >= 3:
        scaler = _SS()
        X = scaler.fit_transform(metricas[["ventas_totales", "venta_promedio", "num_skus"]])
        km = _KM(n_clusters=3, random_state=42, n_init=10)
        metricas["cluster"] = km.fit_predict(X)
        orden = metricas.groupby("cluster")["ventas_totales"].mean().sort_values(ascending=False)
        labels = {c: f"Tienda Tipo {i+1}" for i, c in enumerate(orden.index)}
        metricas["segmento_tienda"] = metricas["cluster"].map(labels)
    else:
        metricas["segmento_tienda"] = "Sin clasificar"

    df_tiendas_seg = metricas[["tienda_id", "ventas_totales", "venta_promedio",
                                "num_skus", "segmento_tienda"]]
    return df_skus, df_tiendas_seg
