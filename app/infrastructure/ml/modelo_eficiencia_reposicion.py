...
def ejecutar_eficiencia(engine) -> "pd.DataFrame":
    """Ejecuta análisis de eficiencia de reposición y retorna DataFrame. No guarda en BD."""
    df_trans, df_inv, df_tiendas = leer_datos(engine)
    df = calcular_eficiencia(df_trans, df_inv, df_tiendas)
    return df[["tienda_id", "cobertura_reposicion", "tasa_devolucion",
               "eficiencia_skus", "indice_eficiencia",
               "clasificacion_eficiencia"]].rename(
        columns={"clasificacion_eficiencia": "clasificacion"}
    )
