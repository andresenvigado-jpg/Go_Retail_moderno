...
# ─────────────────────────────────────────
def ejecutar_pronostico(engine) -> "pd.DataFrame":
    """
    Ejecuta el modelo Prophet y retorna el DataFrame con resultados.
    Recibe un engine SQLAlchemy externo (inyectado desde la API).
    No guarda en BD — eso lo hace el repositorio.
    """
    df_transac = leer_transacciones(engine)
    df_pronostico = entrenar_pronostico(df_transac, dias_pronostico=30, top_skus=10)
    if df_pronostico.empty:
        return df_pronostico
    return df_pronostico.rename(columns={
        "ds": "fecha",
        "yhat": "demanda_estimada",
        "yhat_lower": "demanda_minima",
        "yhat_upper": "demanda_maxima",
